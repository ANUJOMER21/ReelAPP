...
fun NavigationPage(isalreadyLogin:Boolean=false){
    ...
    NavHost(
        ...
        builder = {
            composable(Navigation.Login.route, content = {
                LoginPage(
                    navController
                )
            })
                ...
                composable(
                    Navigation.Reels.route,
                    content = { ReelsScreen(navController = navController) })
                composable(
                    Navigation.Profile.route,
                    content = { ProfilePage(navController = navController) })
                composable(
                    Navigation.AddVideo.route,
                    content = { AddVideo(navController = navController) })
                composable(
                    Navigation.OtpPage.route,
                    content = { OtpPage(navController = navController) })
                composable(
                    Navigation.CategorySelector.route,
                    content = { CategorySelector(navController = navController) })
                composable(
                    Navigation.EditProfile.route,
                    content = { EditProfile(navController = navController) })
                composable(
                    Navigation.Wallet.route,
                    content = { WalletScreen(navController = navController) })
                composable(
                    Navigation.Referearn.route,
                    content = { ReferEarn(navController = navController) })
